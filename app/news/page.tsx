...
export default async function NewsPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const [newsItems, dailySummary, totalCount, pastSummaries, contractCount, criticismCount, selectedBriefing] = await Promise.all([
    ...
    prisma.newsItem.count({ where: whereClause }),
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="lg:col-span-3 space-y-6">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {ALL_TAGS.map((t) => (
              <Link key={t} href={`/news?tag=${t}`} className={`text-xs px-3 py-1.5 rounded-lg border transition-all ${tag === t ? 'bg-orange-900/30 border-orange-700/50 text-orange-200' : 'bg-night-800 border-night-700 text-night-400 hover:text-white'}`}>
                {t}
              </Link>
            ))}
          </div>
          ...
          {featured.length > 0 && (
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {featured.map((item) => (
                  <NewsCard key={item.id} title={item.title} summary={item.summary} aiSummary={item.aiSummary} source={item.source} url={item.url} tags={item.tags as string[]} scrapedAt={item.scrapedAt} featured={true} />
                ))}
              </div>
            </section>
          )}
          ...
          {regular.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {regular.map((item) => (
                <NewsCard key={item.id} title={item.title} summary={item.summary} aiSummary={item.aiSummary} source={item.source} url={item.url} tags={item.tags as string[]} scrapedAt={item.scrapedAt} />
              ))}
            </div>
          ) : (
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 pt-4">
              {pageNum > 1 && (
                <Link href={`/news?${tag ? `tag=${tag}&` : ''}page=${pageNum - 1}`} className="px-4 py-2 bg-night-800 hover:bg-night-700 border border-night-700 text-night-300 rounded-lg text-sm transition-all">← Previous</Link>
              )}
              <span className="text-night-500 text-sm">Page {pageNum} of {totalPages}</span>
              {pageNum < totalPages && (
                <Link href={`/news?${tag ? `tag=${tag}&` : ''}page=${pageNum + 1}`} className="px-4 py-2 bg-night-800 hover:bg-night-700 border border-night-700 text-night-300 rounded-lg text-sm transition-all">Next →</Link>
              )}
            </div>
          )}
        </div>
        ...
        <aside className="space-y-6">
          ...
          <div>
            ...
            <div className="space-y-1">
              {ALL_TAGS.map((t) => (
                <Link key={t} href={`/news?tag=${t}`} className={`flex items-center px-3 py-2 rounded-lg text-sm transition-all ${tag === t ? 'bg-orange-900/20 text-orange-300' : 'text-night-400 hover:text-white hover:bg-night-800'}`}>
                  {t}
                </Link>
              ))}
            </div>
          ...
          <div className="bg-night-900 border border-night-800 rounded-xl p-4">
            ...
            <div className="space-y-1.5 text-xs text-night-500">
              {['Palantir Blog RSS', 'Palantir GitHub API', 'SEC EDGAR RSS', 'SAM.gov Contracts', 'Reddit r/palantir', 'Hacker News API', 'NewsAPI.org', 'YouTube Data API'].map((s) => (
                <div key={s} className="flex items-center gap-2"><span className="text-green-500">●</span> {s}</div>
              ))}
            </div>
