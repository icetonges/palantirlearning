  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <nav className="flex items-center gap-2 text-sm text-night-500 mb-6">
        ...
        <span>›</span>
        <Link href="/ontology" className="hover:text-violet-400 transition-colors text-violet-500">Ontology</Link>
        <span>›</span>
        <span className="text-night-300 truncate">{page.title}</span>
      </nav>
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <span className="text-xs px-2 py-1 bg-violet-900/30 text-violet-300 border border-violet-700/40 rounded font-mono">ONTOLOGY</span>
...
<Link href="/notes?category=ONTOLOGY" className="flex items-center gap-2 px-4 py-2 bg-violet-900/20 hover:bg-violet-800/30 border border-violet-700/40 text-violet-300 rounded-lg text-sm transition-all">+ Add Related Note</Link>
      </div>
    </div>
  )
}
