  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <nav className="flex items-center gap-2 text-sm text-night-500 mb-6">
        ...
        <span>›</span>
        <Link href="/archive" className="hover:text-slate-400 text-slate-500">Archive</Link>
        <span>›</span>
        <span className="text-night-300">{page.title}</span>
      </nav>

      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <span className="text-xs px-2 py-1 bg-night-800 text-night-300 border border-night-700 rounded font-mono">{page.category}</span>
        ...
        <span className="text-xs px-2 py-1 bg-night-800 text-night-500 border border-night-700 rounded font-mono">{page.sourceType}</span>
        <span className="text-night-600 text-xs ml-auto">{formatDate(page.createdAt)} · {page.viewCount} views</span>
      </div>
