...
export default async function ApolloSubPage({ params }: Props) {
  const { slug } = await params
  const page = await prisma.knowledgePage.findUnique({ where: { slug }, include: { flashcards: { select: { id: true } } } })
  if (!page || page.category !== 'APOLLO') notFound()
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <nav className="flex items-center gap-2 text-sm text-night-500 mb-6">
        ...
        <span>›</span>
        <Link href="/apollo" className="hover:text-emerald-400 text-emerald-500">Apollo</Link>
        <span>›</span>
        <span className="text-night-300">{page.title}</span>
      </nav>
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <span className="text-xs px-2 py-1 bg-emerald-900/30 text-emerald-300 border border-emerald-700/40 rounded font-mono">APOLLO</span>
